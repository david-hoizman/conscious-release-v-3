...
const FloatingWhatsAppCTA = () => {
  ...
  // Show prompt after 20 seconds, hide after 10 seconds
  useEffect(() => {
    const showTimer = setTimeout(() => setShowPrompt(true), 20000);
    const hideTimer = setTimeout(() => setShowPrompt(false), 30000);
      ...
      {showPrompt && !isExpanded && (
        <div className="absolute bottom-full left-0 mb-3 animate-fade-in">
          <div className="relative bg-gradient-to-br from-accent/95 to-peach/95 backdrop-blur-sm px-5 py-3 rounded-2xl shadow-xl border border-white/20">
            ...
            <div className="absolute -bottom-2 left-6 w-4 h-4 bg-gradient-to-br from-accent/95 to-peach/95 rotate-45"></div>
          </div>
        </div>
      )}
